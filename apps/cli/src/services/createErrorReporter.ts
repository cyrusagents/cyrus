 ...
 * `ceedar/miko-cli` Sentry project and pastes the DSN here. Sentry DSNs are
 * safe to publish — they only authorise event ingestion.
 *
 * End users may override this with the `MIKO_SENTRY_DSN` env var, or disable
 * reporting entirely with `MIKO_SENTRY_DISABLED=1`.
 */
 ...
 *
 * Order of resolution:
 *   1. If `MIKO_SENTRY_DISABLED` is truthy → noop.
 *   2. If `MIKO_TEAM_ID` is unset → noop. Both Issues and Logs require a
 *      tenant tag so we can slice/filter per Miko install in Sentry; without
...
export function createErrorReporter(
	...
	if (isTruthyEnv(env.MIKO_SENTRY_DISABLED)) {
		return new NoopErrorReporter();
	}

	const tags = buildInitialTags(env);
	// MIKO_TEAM_ID is the single gate for *both* Issues and Logs — installs
	...
	const dsn = env.MIKO_SENTRY_DSN?.trim() || DEFAULT_SENTRY_DSN;
	if (!dsn) {
		return new NoopErrorReporter();
	}
	...
	const environment = env.MIKO_SENTRY_ENVIRONMENT?.trim() || "production";
	...
	return new SentryErrorReporter({
		...
		// MIKO_LOG_LEVEL=DEBUG floods stdout with OpenTelemetry tracing
		// inheritance / client-report flush messages that belong to the SDK,
		// not Miko. Use a dedicated opt-in env var.
		debug: isTruthyEnv(env.MIKO_SENTRY_DEBUG),
		tags,
		...
		sampleRate: parseSampleRate(env.MIKO_SENTRY_SAMPLE_RATE),
		// Always scrub. Miko' logger.error sites pass arbitrary args (request
...
/**
 * Build the structured `miko` context block attached to every event. This is
...
function buildStructuredContext(input: {
	...
	const linearWorkspace = input.env.MIKO_LINEAR_WORKSPACE?.trim();
	if (linearWorkspace) ctx.linear_workspace = linearWorkspace;
	const deployment = input.env.MIKO_DEPLOYMENT_ID?.trim();
 ...
 * `MIKO_TEAM_ID` and exposes it as the `team_id` tag so events can be
 * filtered per Miko tenant in Sentry.
 *
...
function buildInitialTags(
	...
	const teamId = env.MIKO_TEAM_ID?.trim();
