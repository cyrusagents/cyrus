...
export interface SentryErrorReporterOptions {
	 ...
	 * `MIKO_TEAM_ID`). See https://docs.sentry.io/platform-redirect/?next=/enriching-events/tags
	 */
	tags?: Record<string, string>;
	/**
	 * Structured context block attached to every event under the `miko` key.
	 ...
	 * MIKO_LOG_LEVEL — gate on the dedicated `MIKO_SENTRY_DEBUG` env var so
 ...
 *
 * Single Responsibility: this class only knows how to translate Miko-shaped
...
export class SentryErrorReporter implements ErrorReporter {
	...
	constructor(options: SentryErrorReporterOptions) {
		...
		Sentry.init({
			...
			tracesSampleRate: 0,
			// Issues and Logs share a single gate (MIKO_TEAM_ID upstream); by
			...
			//     `err.requestId`, custom Miko error fields surface in Sentry).
			...
			// Apply caller-provided tags (e.g. team_id) and a structured `miko`
	...
	log(
		...
		// (and any other MIKO_* tag we configured) lands on every log record.
...
function buildInitialScope(
	...
	return {
		...
		...(hasContext ? { contexts: { miko: options.structuredContext } } : {}),
	};
}
