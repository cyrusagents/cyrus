...
- **Per-Prompter Credentials (multi-user self-host)**: `packages/core/src/prompter-credentials.ts` (credential references, policy decisions, per-session env) + `packages/edge-worker/src/PrompterCredentialService.ts` (webhook → prompter, session pin, refusal messages) + `apps/cli/src/commands/UserCommands.ts` (`add-user` / `list-users` / `check-users` / `remove-user`). `EdgeConfig.linearUsers` holds references only; personal PATs share `~/.cyrus/github-tokens.json` with installation tokens, while Claude secrets remain under `~/.cyrus/user-credentials/` or in env vars. The shared store uses cross-process locked atomic updates; its `personalTokens` map is locally owned and preserved on hosted refreshes. The mapping is local to the box — `packages/config-updater` carries it across cyrus-hosted config pushes (`LOCALLY_OWNED_KEYS`), and cyrus-hosted's `builder.emitted-keys.test.ts` pins that hosted never emits it. All runners get personal GitHub identity via `AgentRunnerConfig.additionalEnv` / `omitEnv`; Claude optionally receives personal model authentication, and the others retain their existing model auth. Cursor uses a worker environment; Codex pools app-server processes by environment.
