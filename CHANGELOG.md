...
### Added
- Multi-user self-host: map each Linear user to their **own** Claude credential, GitHub token and Git identity so sessions they trigger run as them. `cyrus add-user --linear-email <email>` takes the tokens via hidden prompt, protected file or env-var name (never a command-line argument), verifies them against Claude and GitHub, stores them as owner-only files under `~/.cyrus/user-credentials/`, and writes only references into `config.json` (`linearUsers`). Each session then gets its own environment — the user's `CLAUDE_CODE_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`, `GH_TOKEN`, git author/committer identity and a per-session git credential helper for `github.com` (SSH remotes rewritten to HTTPS) — with the host's Claude credential removed, so `gh pr create` opens PRs as the triggering human and no global `gh`/git state changes. Unmapped humans, non-human triggers without a mapped parent session, and unreadable/expired mappings are refused with an explanatory Linear response instead of silently using the host's credentials; sub-issues inherit the parent session's user; a follow-up from another user keeps the session pinned to the original user with a visible note (`prompterCredentialPolicy.followUpByOtherUser` can reject instead). `cyrus list-users`, `cyrus check-users [--live]` and `cyrus remove-user` manage the mapping, which hot-reloads and survives cyrus-hosted config pushes. Claude and OpenCode runners only; see the docs for the full policy table and platform limits. ([CYPACK-1502](https://linear.app/ceedar/issue/CYPACK-1502/resolve-claude-credentials-and-gitlinear-identities-by-prompter-for))
