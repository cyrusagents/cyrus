...
- **Codex sessions now complete cleanly without tripping the idle watchdog** — Added a Tier A prompt-level fix that injects a `<linear_write_constraints>` block into every runner prompt, instructing the model not to call `mcp__linear__save_issue` or `mcp__linear__save_comment` from within its turn (these mutations hang the codex runtime indefinitely). Cyrus's orchestrator continues to drive Linear state from the result side. Includes reproducible patch, idempotent install/uninstall script with SHA gating, and updated runbook. Takes Tier 2 from fail-fast to shipping-grade. ([BRI-1518](https://linear.app/brilliantio/issue/BRI-1518/fix-codex-completion-gap-strip-linear-write-calls-from-codex-prompt))

### Added
- **Bridge work-lease handoff support** — Cyrus now detects a `CYRUS-MACHINE-HANDOFF` marker in issue comments and, when present, atomically adopts and verifies a bridge work lease with the authority (POST adopt → GET readback) before any issue-state transition, worktree creation, or session mutation. The `starting_sha` from the verified lease is bound as a base-branch override so the new session continues from exactly where the bridge agent left off. Startup is fully fail-closed: any ambiguity (invalid marker, authority error, binding mismatch) throws before any mutation. The legacy startup path (no marker) is unchanged. Configured via `CYRUS_WORK_LEASE_URL`, `CYRUS_WORK_LEASE_TOKEN`, `CYRUS_WORK_LEASE_PRINCIPAL_ID`, and `CYRUS_WORK_LEASE_TTL_SECONDS`. ([BRI-3257](https://linear.app/brilliantio/issue/BRI-3257), [#1382](https://github.com/cyrusagents/cyrus/pull/1382))
