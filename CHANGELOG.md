...
- **OpenCode Linear transcripts show useful model names** — OpenCode sessions now report the selected provider/model from Cyrus or OpenCode runtime config instead of the internal `opencode` placeholder in Linear activity timelines. ([NG-157](https://linear.app/digimondo/issue/NG-157/show-useful-opencode-providermodel-names-in-linear-transcripts))
