...
- The per-session skill allowlist is now built from the issue worktree's `.claude/skills/` instead of the persistent base checkout Cyrus uses to create worktrees — the same class of fix as the setup/teardown hook change above. Previously, single-repo (and GitHub-mention) sessions resolved repo-local skills from `repository.repositoryPath`, whose working tree Cyrus never advances (it only fetches and cuts worktrees from `origin/<base>`). Any skill committed to the base branch after a repo was registered was offered to the model by the SDK (which loads skills from the fresh worktree) but then rejected on use with `Skill <name> is not in this session's skills allowlist`. Skill discovery now uses `workspace.path`, matching the worktree the SDK actually loads from. ([#1336](https://github.com/cyrusagents/cyrus/issues/1336), [#1337](https://github.com/cyrusagents/cyrus/pull/1337))
