...
### Fixed
- **Sessions no longer fail to start with `spawn E2BIG` after long uptime** — On long-running deployments, every per-issue directory under `~/.cyrus/` accumulated as a separate `Read(...)` deny pattern in the disallowed-tools list. Once a few thousand had accumulated, the resulting argv exceeded the OS `ARG_MAX` limit and every new Claude session failed to spawn — making issues appear "stuck" in the started state with no final response posted. When an issue moves to Done or Cancelled, Cyrus now removes the per-issue `~/.cyrus/<issue-id>/` directory alongside the existing worktree cleanup, keeping the deny-list size bounded.
