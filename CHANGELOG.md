...
### Fixed
- **Linear OAuth refresh tokens are now significantly more durable** — A class of failure where the refresh token ended up dead (`invalid_grant` permanently) silently took out 8 of 13 agents in one Tincture deployment over a few weeks. Root cause: concurrent or crash-interrupted refreshes against Linear's rotating-refresh-token protocol. Five layered defenses now ship:
  - **Per-host advisory file lock** on the credentials file wraps the refresh call, so sibling processes sharing a config can't race `/oauth/token`. (`proper-lockfile`, cross-platform.)
  - **Disk re-read after lock acquisition** — if a peer rotated the token while we were queued, we adopt their new token instead of issuing a stale request.
  - **Replay sidecars** — the outbound refresh request is persisted to disk before being sent. On startup, recent sidecars (within Linear's documented 30-minute idempotency window) are replayed to recover from a crash between sending and persisting. The same request returns the same new tokens.
  - **Atomic rename** on the workspace-config write (temp file + `rename`) so a crash mid-persist can't corrupt or truncate the credentials file.
  - **Proactive windowed refresh** — a periodic monitor refreshes any token that has <5 min left before expiry, eliminating the thundering-herd concurrent-401 path that triggers the rotation race in the first place.
- **Dead-token state now surfaces within minutes instead of days** — A new `TokenHealthMonitor` runs a lightweight `viewer` query against every workspace every 5 minutes and emits ERROR-level logs on failure. External monitors (e.g., journal-based) can grep for these to alert operators well before the next webhook would have revealed the problem.
- **`invalid_grant` is now treated as terminal** — Per RFC 9700 §4.14, an auth server that detects refresh-token reuse revokes the entire token family. Cyrus now logs an operator-actionable error with the re-auth command instead of looping on retries (which compound the damage).
...
- **IPv6 webhook deliveries are no longer silently dropped** — `WebhookIpValidator.ipMatchesAllowlist` previously rejected any non-IPv4 source IP because the configured allowlists only carry IPv4 CIDRs. Linear has been progressively shifting webhook delivery to IPv6, so a growing share of deliveries failed at the validator with `Rejected … unauthorized IP: 2a…` and never reached the agent. The validator now passes non-IPv4 IPs through; allowlist validation still applies for IPv4.
...
- **Optional `linearAgentId` / `linearAgentName` config fields** — Pin the Linear identity an instance uses to self-filter Project-level @-mentions. When omitted, the identity is auto-resolved from the Linear `viewer` query at startup. ([#1213](https://github.com/cyrusagents/cyrus/pull/1213))
- **Agent deliverables are now auto-attached to the Linear issue** — When an issue-bound agent session finishes, any deliverable files it created or changed in its worktree (`.md`, `.pdf`, `.docx`, `.csv`, `.xlsx`, images — not source code, config, or build noise) are uploaded to Linear and attached to the originating issue. No more digging through the git branch or PR to find what the agent produced. Detection covers both uncommitted changes and files committed on the session branch (vs. the base branch), so it works whether or not the agent ran verify-and-ship before finishing. The hook also fires for sessions that end as "stopped by user" — not just clean completions — because a conversational issue's turns almost always end that way (the next reply supersedes the current turn); it dedups against attachments already on the issue so iterative work doesn't pile up duplicates. Best-effort (a failure never blocks the session) and worktree-only — Slack and Project Update chat sessions are unaffected. ([#1214](https://github.com/cyrusagents/cyrus/pull/1214))
