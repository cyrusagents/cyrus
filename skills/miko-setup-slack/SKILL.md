---
name: miko-setup-slack
description: Configure Slack integration for Miko — create a Slack app from manifest, then guide the user to save credentials.
---
...
Creates a Slack application from a pre-built manifest so Miko can respond to messages in Slack channels.

## Step 1: Check Existing Configuration

```bash
grep -E '^SLACK_BOT_TOKEN=' ~/.miko/.env 2>/dev/null
```

If `SLACK_BOT_TOKEN` is already set, inform the user:

> Slack is already configured. Skipping this step.
> To reconfigure, remove `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET` from `~/.miko/.env` and re-run.
...
Construct the manifest, substituting `<AGENT_NAME>`, `<AGENT_DESCRIPTION>`, and `<MIKO_BASE_URL>` with actual values.

**IMPORTANT: Use the manifest template EXACTLY as shown below.** The event subscription path MUST be `/slack-webhook` (not `/slack/events` or any other path). This matches the route registered by `SlackEventTransport` in the Miko codebase.

**Keep this scope list in sync with the manifest below and your own Slack App configuration.**
...
```json
{
    ...
    "oauth_config": {
        "redirect_urls": [
            "<MIKO_BASE_URL>/slack/oauth/callback"
        ],
        "scopes": {
            "user": [
                "canvases:read",
    ...
    "settings": {
        "event_subscriptions": {
            "request_url": "<MIKO_BASE_URL>/slack-webhook",
            "bot_events": [
                "app_mention",
...
```bash
grep -q '^SLACK_BOT_TOKEN=' ~/.miko/.env || echo 'SLACK_BOT_TOKEN=' >> ~/.miko/.env
grep -q '^SLACK_SIGNING_SECRET=' ~/.miko/.env || echo 'SLACK_SIGNING_SECRET=' >> ~/.miko/.env
```
...
> I've opened `~/.miko/.env`. You need to paste two values:
>
...
```bash
grep -c '^SLACK_BOT_TOKEN=.' ~/.miko/.env
grep -c '^SLACK_SIGNING_SECRET=.' ~/.miko/.env
```
...
> ✓ Bot token and signing secret saved to `~/.miko/.env`

**Note:** The event subscription `request_url` will fail Slack's verification challenge until Miko is actually running. Once Miko is started, go to the app's **Event Subscriptions** page and re-enter the URL to trigger verification, or Slack will retry automatically.
