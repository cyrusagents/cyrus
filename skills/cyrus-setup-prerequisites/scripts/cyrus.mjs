...
export async function verifyRelease(directory) {
	...
	const edgeWorker = await realpath(
		createRequire(entrypoint).resolve("cyrus-edge-worker"),
	);
	const core = await realpath(createRequire(edgeWorker).resolve("cyrus-core"));
	...
	if (edgeWorker !== expectedWorker || core !== expectedCore)
		throw Error(
			"Cyrus dependencies must resolve to the built workspace packages",
		);
	for (const path of [edgeWorker, core]) {
		if (!inside(root, path))
			throw Error(
				"A Cyrus workspace dependency resolves outside the source release: " +
					path,
			);
	}
	for (const path of [
...
export async function readInstallation(root) {
	...
	if (
		info.repository !== "https://github.com/nexmoe/cyrus.git" ||
...
async function main() {
	...
	// Import in this process so Cyrus owns signals and its normal shutdown hooks.
...
if (
	...
	main().catch((error) => {
		console.error(`Cyrus fork: ${error.message}`);
		process.exitCode = 1;
	});
}
