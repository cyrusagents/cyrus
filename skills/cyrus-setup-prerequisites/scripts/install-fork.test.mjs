...
async function fixture() {
	const root = await mkdtemp(join(tmpdir(), "cyrus-fork-test-"));
	temporary.push(root);
	...
	for (const name of ["core", "edge-worker"]) {
		...
		await writeFile(
			...
			JSON.stringify({ name: `cyrus-${name}`, main: "dist/index.js" }),
		);
		await writeFile(join(pkg, "dist/index.js"), "");
		await symlink(
			pkg,
			join(release, `node_modules/cyrus-${name}`),
	...
	const info = {
		repository: "https://github.com/nexmoe/cyrus.git",
		commit: "a".repeat(40),
		release: "source-test",
	};
	await writeFile(join(root, "current.json"), JSON.stringify(info));
	await copyFile(join(scripts, "cyrus.mjs"), join(root, "cyrus.mjs"));
...
test("the launcher verifies workspace artifacts and forwards CLI arguments", async () => {
	...
	const result = spawnSync(
		process.execPath,
		[join(root, "cyrus.mjs"), "--cyrus-home", "a path with spaces", "start"],
		{ encoding: "utf8" },
	);
...
test("published-package fallback inside the checkout is rejected", async () => {
	...
	await writeFile(
		...
		'{"name":"cyrus-edge-worker","main":"index.js"}',
	);
	await writeFile(join(substitute, "index.js"), "");
	const link = join(release, "node_modules/cyrus-edge-worker");
	await unlink(link);
	await symlink(
		substitute,
		link,
