...
import { readInstallation, verifyRelease } from "./cyrus.mjs";
...
const defaultRoot = windows
	...
	: join(homedir(), ".local/share/cyrus-nexmoe");
	...
	try {
		...
		if (
			...
			if (previous.commit === values.ref) {
				await atomicWrite(
					join(root, "cyrus.mjs"),
					await readFile(join(scripts, "cyrus.mjs")),
				);
				console.log(`Already installed and verified: ${previous.commit}`);
				console.log(`Launcher: ${join(root, "cyrus.mjs")}`);
				return;
			}
		}
		...
		if (windows) {
			...
			const bash =
				process.env.CYRUS_BUILD_SHELL ||
				resolve(gitExecPath, "../../..", "bin/bash.exe");
			if (!existsSync(bash))
				throw Error(
					"Git Bash is required. Install Git for Windows or set CYRUS_BUILD_SHELL to bash.exe.",
				);
		...
		pnpm(version, ["--filter", "cyrus-ai...", "build"], checkout, env);
		...
		run(
			process.execPath,
			[entrypoint, "--cyrus-home", verificationHome, "--version"],
			checkout,
			{ ...env, CYRUS_HOME: verificationHome, CYRUS_SENTRY_DISABLED: "1" },
		);
		const info = {
		...
		console.log(`Installed nexmoe/cyrus @ ${commit}`);
		console.log(`Launcher: ${join(root, "cyrus.mjs")}`);
		console.log(`Verify: node "${join(root, "cyrus.mjs")}" --installation`);
		console.log(`Start:  node "${join(root, "cyrus.mjs")}" start`);
		console.log(
