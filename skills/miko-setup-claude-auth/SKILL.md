---
name: miko-setup-claude-auth
description: Configure Claude Code authentication for Miko — API key, OAuth token, or third-party provider.
---
...
Configures Claude Code credentials so Miko can run AI sessions.
...
```bash
grep -c -E '^(ANTHROPIC_API_KEY|CLAUDE_CODE_OAUTH_TOKEN)=' ~/.miko/.env 2>/dev/null || echo "0"
```

If the count is >= 1, inform the user:

> Claude Code authentication is already configured. Skipping this step.
> To reconfigure, remove the existing key from `~/.miko/.env` and re-run this skill.
...
```bash
grep -q '^CLAUDE_CODE_OAUTH_TOKEN=' ~/.miko/.env || echo 'CLAUDE_CODE_OAUTH_TOKEN=' >> ~/.miko/.env
```
...
# Linux
code ~/.miko/.env 2>/dev/null || xdg-open ~/.miko/.env
```
...
> 3. I've opened `~/.miko/.env` — find the `CLAUDE_CODE_OAUTH_TOKEN=` line
...
**Linux:**
```bash
printf 'ANTHROPIC_API_KEY=%s\n' "$(xclip -selection clipboard -o)" >> ~/.miko/.env
```
...
> 2. Copy the token it outputs
> 3. I've opened `~/.miko/.env` — find `CLAUDE_CODE_OAUTH_TOKEN=` and paste the token after the `=`
> 4. Save and close
...
Guide the user to add the appropriate variables to `~/.miko/.env` using the clipboard-to-env pattern.
...
```bash
grep -c -E '^(ANTHROPIC_API_KEY|CLAUDE_CODE_OAUTH_TOKEN|CLAUDE_CODE_USE_BEDROCK|CLAUDE_CODE_USE_VERTEX)=' ~/.miko/.env
```
