---
name: miko-setup-prerequisites
description: Install the mikoagents/miko fork from source, verify its workspace packages and status board, and provide the launcher used by the remaining setup steps.
---
...
Checks system prerequisites and builds the mikoagents/miko fork. This skill includes its installer under `scripts/`; run that copy even when the skills were installed without a Git checkout.
...
Preserve any package-manager preference for unrelated tools, but use the locked pnpm workspace build for Miko. An existing global `miko` command or matching npm version is not proof that this fork is installed.
...
On Windows, use Git for Windows with Git Bash: repository build scripts use `cp` and `rm`. The installer discovers Git Bash from `git --exec-path`; `MIKO_BUILD_SHELL` can specify its absolute `bash.exe` path if the layout differs. Use the equivalent PowerShell checks on Windows.
...
The default install directory is `%LOCALAPPDATA%/Miko` on Windows and `~/.local/share/miko` on macOS/Linux. It is separate from `~/.miko`, which retains credentials, repositories and history. For a user-selected directory, pass `--install-dir "<directory>"` on every install/update.

The installer fetches the configured source ref, installs the frozen lockfile, builds the CLI and its workspace dependencies, verifies that `miko-edge-worker` and `miko-core` resolve inside this source release, and checks the board assets. It activates the release only after these checks succeed. It leaves global commands and existing workers untouched.

Keep the printed launcher path as `MIKO_ENTRY` for all remaining steps. For example, set `$MIKO_ENTRY = '<printed path>'` in PowerShell or `MIKO_ENTRY='<printed path>'` in Bash. Use `node "$MIKO_ENTRY"` for **every Miko CLI call**, including authentication, repository setup and service startup. Pass this path to each sub-skill; do not fall back to a bare `miko` command.
...
Check that the reported repository is `https://github.com/mikoagents/miko.git` and the resolved commit matches the selected source ref (use `--ref <commit>` for an immutable install). The npm version alone cannot distinguish the fork.
...
If `~/.miko/.env` already exists, note it and inform the user that existing values will be preserved.
...
Prerequisites:
  ...
  ✓ jq 1.7
  ✓ gh 2.x
  ✓ mikoagents/miko source commit verified
  ✓ Fork launcher: <MIKO_ENTRY>
