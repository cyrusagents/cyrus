...
PATCH_FILE="${REPO_ROOT}/patches/cyrus-codex-runner-idle-timeout.patch"

EXPECTED_PRE_SHA="db8f556b13811a1f97e464fb6dbfa90969cd06fd9fb23e1f8f7d8f252906b2f3"
EXPECTED_POST_SHA="1ccc2ee554985b50dd8d43dd4134445520176cb40e49fb79d4827ca9e95b0a43"
...
log() {
  printf '[install-codex-runner-patch] %s\n' "$*" >&2
}

die() {
  printf '[install-codex-runner-patch] ERROR: %s\n' "$*" >&2
  exit 1
}
...
find_target_file() {
  local npm_root
  npm_root="$(npm root -g)"
  local candidate="${npm_root}/cyrus-ai/node_modules/cyrus-codex-runner/dist/CodexRunner.js"
  ...
  discovered="$(find /usr/lib/node_modules /usr/local/lib/node_modules \
    -path '*/cyrus-ai/node_modules/cyrus-codex-runner/dist/CodexRunner.js' \
...
restore_latest_backup() {
  ...
  restart_pm2
  log "Uninstall complete. Runner restored to pre-patch SHA ${EXPECTED_PRE_SHA}."
}
...
apply_patch_safely() {
  ...
  restart_pm2
  log "Install complete. Runner now at post-patch SHA ${EXPECTED_POST_SHA}."
...
[[ -n "${TARGET_FILE}" ]] || die "Could not find cyrus-codex-runner CodexRunner.js"
