...
### Added
- Per-thread Codex sandbox via the app-server. A new `resolveCodexSandbox` unit maps Cyrus sandbox intent to a transport-neutral `ResolvedCodexSandbox` union (`workspace-mode` for the default coarse mode, or a granular `policy`); `AppServerCodexBackend` serializes it (`thread/start.sandbox` + config for the mode arm; `turn/start.sandboxPolicy` for the policy arm, which persists per-thread). `RunnerConfigBuilder` plumbs the egress sandbox's filesystem posture (worktree-write, allowed-dirs-read, home denied by omission) into `CodexRunnerConfig.sandboxSettings` for Codex, mirroring Claude. Sandbox logic was consolidated out of `CodexConfigBuilder`/the backend into the single `resolveCodexSandbox` owner; `ResolvedCodexConfig.additionalDirectories` was folded into the sandbox union's writable roots. Phase-0 validation against the real binary established the mechanism: the granular `permissionProfile` (managed/restricted) is unusable — it has no platform-defaults escape hatch and starves the shell — so the structured `sandboxPolicy` (with `readOnlyAccess.includePlatformDefaults`) is used instead; enforcement (allowed write succeeds, outside write blocked) was confirmed end-to-end through `CodexRunner`.
