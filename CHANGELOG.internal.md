...
### Added
- Diagnosed DataImpulse proxy (port 823) vs Webshare proxy success/failure for Amazon UK scraping. Root cause: IP rotation luck — Webshare hit a clean residential IP in BRI-889; DataImpulse consistently rotated to flagged IPs. Key findings: (1) `proxyUsed: basic` is a Firecrawl-internal label, not an indicator of which proxy was used; (2) Playwright headless browser is always detected by Amazon regardless of proxy; (3) Firecrawl's `fetch` fallback also returns 503 through both proxies; (4) VPS direct IP (156.67.216.89 Hostinger SG) gets Amazon 200 OK with browser UA; (5) Webshare works ~33% of requests when IP rotates to a clean node. Config reverted to Webshare (`p.webshare.io:80`). Recommendation: add 2–3 retry logic for 5xx Amazon responses in the n8n pipeline. ([BRI-891](https://linear.app/brilliantio/issue/BRI-891))
- Swapped Webshare residential proxy for DataImpulse residential rotating proxy (`gw.dataimpulse.com:823`) in Firecrawl on the VPS — DataImpulse costs $1/GB vs Webshare $3.50/GB (3.5× cheaper). Updated `.env.example` placeholder values to document the DataImpulse proxy format. Proxy test confirmed working (residential IP returned). Amazon UK scrape still returns HTTP 503 (bot-detection); Firecrawl reports `proxyUsed: basic` suggesting its own proxy-routing layer may not be passing the configured `PROXY_SERVER` for all scrape types. n8n workflow end-to-end test ran (topic="british trivia quiz books", market=uk, max_results=10) and completed; report written with 0 listings due to Amazon blocking. ([BRI-890](https://linear.app/brilliantio/issue/BRI-890), [#13](https://github.com/Brilliantio/cyrus-agent/pull/13))
- Swapped Webshare free datacenter proxy for Webshare residential rotating proxy (`p.webshare.io:80`, `-rotate` suffix) in Firecrawl on the VPS. Updated `.env.example` placeholder values to document the residential proxy format. Fixed VPS DNS resolution (`p.webshare.io` was NXDOMAIN on the default systemd-resolved; fixed by setting `DNS=8.8.8.8` in `/etc/systemd/resolved.conf`). Full end-to-end workflow test completed in topic mode (`topic="documentary filmmaking"`, `market="uk"`, `max_results=10`): proxy test returned residential IP, Amazon UK search returned 188,661 chars/37 ASINs, 10/10 product pages scraped, Sonnet 4 analysis complete. ([BRI-889](https://linear.app/brilliantio/issue/BRI-889), [#12](https://github.com/Brilliantio/cyrus-agent/pull/12))
...
- Added Branch 2.75 in `EdgeWorker.handleUserPromptedAgentActivity()` for elicitation responses, with `handleElicitationResponse()` and `handleTestFailureElicitation()` methods. ([BRI-857](https://linear.app/brilliantio/issue/BRI-857), [#2](https://github.com/Brilliantio/cyrus-agent/pull/2))
- Made `clearValidationLoopState()` public on `AgentSessionManager` for EdgeWorker access during elicitation skip flow. ([BRI-857](https://linear.app/brilliantio/issue/BRI-857), [#2](https://github.com/Brilliantio/cyrus-agent/pull/2))
- Added 17 tests for `ElicitationManager` covering posting, timeout, response handling, cancellation, file persistence, and disposal. ([BRI-857](https://linear.app/brilliantio/issue/BRI-857), [#2](https://github.com/Brilliantio/cyrus-agent/pull/2))
