...
- `docs/codex-completion-gap-diagnosis.md` — Tier-2 shipping-grade follow-on diagnosis. Establishes via JSONL evidence from `/root/.codex/sessions/` that the BRI-1489 watchdog-fired failure is the *same* Linear-MCP mutation hang as BRI-1410 (both ended on orphan `mcp__linear__save_issue` / `save_comment` calls), not a missing terminal-event handler. Confirms `cyrus-codex-runner` already correctly maps `task_complete` → `turn.completed` at `dist/CodexRunner.js:663`. Recommends Tier A: prompt-level mitigation in `packages/edge-worker/src/PromptBuilder.ts` to instruct codex not to call `mcp__linear__save_issue` / `save_comment` (Cyrus EdgeWorker handles state transitions and final comments). Diagnosis only; no fix in this PR. ([BRI-1490](https://linear.app/brilliantio/issue/BRI-1490))
- Added BRI-1439 Tier A/Tier B operational artifacts: `patches/cyrus-codex-runner-idle-timeout.patch`, `scripts/install-codex-runner-patch.sh`, `config/codex-config.toml`, `scripts/install-codex-config.sh`, and `docs/codex-runner-fix-runbook.md`. Includes deterministic SHA checks, dry-run/uninstall paths, and verification logging guidance for codex-runner hang mitigation. ([BRI-1439](https://linear.app/brilliantio/issue/BRI-1439/fix-codex-runner-hang-idle-timeout-watchdog-per-mcp-tool-timeout-sec), [#33](https://github.com/Brilliantio/cyrus-agent/pull/33))
