...
/**
 * Options for creating Miko tools with session management capabilities
 */
export interface MikoToolsOptions {
	/**
...
/**
 * Create a standard MCP SDK server with Miko tools.
 */
export function createMikoToolsServer(
	linearClient: LinearClient,
	options: MikoToolsOptions = {},
): McpServer {
		...
		async ({ agentSessionId, message }) => {
			...
			if (options.onFeedbackDelivery) {
				try {
					...
					console.error("[MikoTools] Failed to deliver feedback:", error);
				}
			}
	...
	// miko-hosted). Omitted in CLI mode where there is no control plane.
