...
export interface FetchFailureModesClientOptions {
	/**
	 * Base URL of the miko-hosted control plane (e.g.
	 * `https://control-plane.example.com`). Trailing slashes are tolerated.
	 */
	baseUrl: string;
	/**
	 * The `MIKO_API_KEY` bearer token. Auth is reverse-looked-up server-side
	 * against both `miko_api_key` (self-host) and `droplet_api_key_encrypted`
	 * (cloud) columns.
	 */
	apiKey: string;
