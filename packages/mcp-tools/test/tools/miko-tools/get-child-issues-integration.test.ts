...
import { createMikoToolsServer } from "../../../src/tools/miko-tools/index.js";

describe("linear_get_child_issues tool integration", () => {
	it("should create miko tools server with expected structure", () => {
		const server = createMikoToolsServer({} as any);
	...
	it("should verify tool exists through successful server creation", () => {
		...
		expect(() => createMikoToolsServer({} as any)).not.toThrow();

		// Create another server with options to ensure configuration works
		expect(() =>
			createMikoToolsServer({} as any, {
	...
	it("should handle different configuration options", () => {
		...
		const server1 = createMikoToolsServer({} as any);
		expect(server1).toBeDefined();

		// Test with full config
		const server2 = createMikoToolsServer({} as any, {
			parentSessionId: "parent-456",
		});
		expect(server2).toBeDefined();

		// Test with empty options
		const server3 = createMikoToolsServer({} as any, {});
