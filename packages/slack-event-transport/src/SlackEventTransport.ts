...
export class SlackEventTransport extends EventEmitter {
	...
	private processAndEmitEvent(
		...
		// the MIKO_SLACK_THREAD_FOLLOWING_DISABLED env kill-switch). When off,
		...
		// check (is this thread actually bound to Miko?) happens downstream.
	 ...
	 * threaded reply can belong to a thread Miko is already bound to.
	 */
