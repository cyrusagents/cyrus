...
import { contentTypeForFile } from "./deliverables.js";
...
export class EdgeWorker extends EventEmitter {
	...
	constructor(config: EdgeWorkerConfig) {
		...
		this.agentSessionManager = new AgentSessionManager(
			...
			undefined, // logger — AgentSessionManager creates its own
			(sessionId, session) =>
				this.handleSessionDeliverables(sessionId, session),
		);
...
${taskSection}`;
	...
	/**
	 * Auto-attach an agent's deliverable files to the originating Linear issue
	 * (post-session hook, fired by AgentSessionManager on successful completion).
	 *
	 * Detects the deliverable files the agent created/modified in its worktree
	 * (`.md` specs, generated docs, images — not code or build noise), uploads
	 * them to Linear, and attaches them to the issue — so they're one click
	 * away on the issue instead of buried in a git branch or PR. Best-effort
	 * throughout: every failure is logged and swallowed, and worktree-free
	 * sessions (Slack, Project Update chats) no-op immediately.
	 */
	private async handleSessionDeliverables(
		sessionId: string,
		session: CyrusAgentSession,
	): Promise<void> {
		// Only issue-bound sessions with a real git worktree have deliverables.
		if (!session.workspace.isGitWorktree) return;

		const issueId = session.issueContext?.issueId ?? session.issueId;
		if (!issueId) return;

		// Resolve the Linear issue tracker for this session's repository.
		...
		if (!tracker) {
			this.logger.debug(
				`No issue tracker for session ${sessionId} — skipping deliverable attach`,
			);
			return;
		}

		// One worktree per repo for multi-repo sessions, else the single path.
		const worktreePaths =
			session.workspace.repoPaths &&
			Object.keys(session.workspace.repoPaths).length > 0
		...
		for (const filePath of files.slice(0, MAX_FILES)) {
			try {
				...
				if (buf.byteLength > MAX_BYTES) {
					this.logger.warn(
						`Deliverable ${filePath} is ${buf.byteLength} bytes (> ${MAX_BYTES}) — skipping attach`,
					);
					continue;
				}
				...
				if (!putRes.ok) {
					this.logger.warn(
						`Upload PUT for ${filename} → HTTP ${putRes.status} — skipping`,
					);
					continue;
				}
				await tracker.createAttachment({
					issueId,
					title: filename,
					url: upload.assetUrl,
					subtitle: "Generated by Cyrus",
				});
				attached++;
			} catch (error) {
				this.logger.warn(
					`Failed to attach deliverable ${filePath} to issue ${issueId}: ${
						error instanceof Error ? error.message : String(error)
					}`,
				);
			}
		...
		if (attached > 0) {
			this.logger.info(
				`Attached ${attached} deliverable file(s) to issue ${issueId} (session ${sessionId})`,
			);
		}
	}
