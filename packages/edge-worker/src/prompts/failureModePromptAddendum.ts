...
You have access to the MCP tool \`mcp__miko-tools__log_failure_mode\`. Use it to self-report material, user-visible agent failures so the Miko team can intervene before a user loses trust or churns.
...
- Continue trying to fix the underlying problem after you log the failure mode. Logging is a signal to the Miko team; it is not a substitute for resolving the user's request.
