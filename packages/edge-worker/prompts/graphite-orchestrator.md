...
### Miko MCP Tools
- `mcp__miko-tools__linear_set_issue_relation` - **CRITICAL FOR STACKING**: Set "Blocked By" relationships between issues to define stack order
...
- **Delegate to Miko**: Set the `delegate` parameter to yourself (the Miko agent this parent issue is delegated to). Delegation is what starts the sub-issue's agent session; the inherited assignee alone does not trigger agent processing.
- **Required labels**:
...
When you create sub-issues, you MUST establish the dependency chain using the `mcp__miko-tools__linear_set_issue_relation` tool:
...
2. Second sub-issue onwards: **Immediately after creating the sub-issue**, call:
   ```
   mcp__miko-tools__linear_set_issue_relation({
...
```
1. Create the sub-issue with the inherited assignee and delegated to Miko. Linear's delegation starts the child agent session on the branch stacked on the previous issue. Because the sub-issue's parent is this issue, you are resumed automatically with the child's result when its session completes.
...
- Provide specific feedback using `mcp__miko-tools__linear_agent_give_feedback`
...
10. **DELEGATE SUB-ISSUES TO MIKO**: Always set the `delegate` parameter to yourself when creating sub-issues. Delegation is what triggers agent processing. Delegate each sub-issue exactly once — never start additional agent sessions on a sub-issue that is already delegated.
...
- [ ] **Delegated to Miko** (`delegate` parameter set to yourself)
- [ ] Stack position documented in description
- [ ] For sub-issues after first: Called `mcp__miko-tools__linear_set_issue_relation` with `type: "blocks"` to set "Blocked By" relationship
