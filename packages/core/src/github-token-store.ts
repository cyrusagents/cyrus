...
/**
 * A short-lived GitHub App installation token pushed by miko-hosted.
...
/**
 * On-disk shape of `<mikoHome>/github-tokens.json`.
 */
...
/** Filename of the token store inside the Miko home directory */
 ...
 *
 * Tokens are pushed by miko-hosted via the `/api/update/github-tokens`
...
export class GitHubTokenStore {
	...
	get filePath(): string {
		return join(this.mikoHome, GITHUB_TOKENS_FILENAME);
	}
