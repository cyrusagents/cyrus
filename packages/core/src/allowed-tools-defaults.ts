 ...
 *
 * These are the single source of truth for "what tools does Miko have access
 * to when a session is triggered by platform X". miko-hosted and any
 ...
 * workspace MCP tools at runtime. Anything Miko needs (including
 * `mcp__linear`, `mcp__miko-tools`, `mcp__miko-docs`, `mcp__slack`, and
 * read access to repository paths) is listed here explicitly. If you remove
 * a tool from this list, Miko loses access to it. If you add a tool here,
 ...
 *
 * Linear sessions are full engineering sessions — Miko opens worktrees,
 * runs builds, edits files, and opens PRs. This list mirrors the full
 * Claude Agent SDK toolset plus the workspace MCP prefixes Miko needs
...
export const LINEAR_DEFAULT_ALLOWED_TOOLS = [
	...
	// sessions include `mcp__slack` so Miko can post status updates and
 ...
 * The default list grants read-only access to repository sources (so Miko
 ...
 *
 * GitHub sessions are full engineering sessions like Linear (Miko opens
