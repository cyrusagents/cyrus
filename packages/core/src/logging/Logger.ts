...
function parseLevelFromEnv(): LogLevel | undefined {
	const envLevel = process.env.MIKO_LOG_LEVEL?.toUpperCase();
	switch (envLevel) {
		case "DEBUG":
...
class Logger implements ILogger {
	 ...
	 * reporter only ships logs when MIKO_TEAM_ID is configured).
	 */
	...
	private forwardToErrorReporter(message: string, args: unknown[]): void {
		...
		// Start with process-wide tags (e.g. team_id from MIKO_TEAM_ID) so they
