...
import type { McpServerConfig } from "miko-core";
...
function applyMikoMcpAllowedToolsSemantics(
	...
	// Codex separates tool visibility (`enabled_tools`) from MCP approval. Miko
...
/**
 * Translate Miko MCP server configs (file-based + inline) and Miko
...
export function buildCodexMcpServersConfig(
	...
	for (const [serverName, rawConfig] of Object.entries(mergedServers)) {
		...
		if (allowedToolsFilter) {
			applyMikoMcpAllowedToolsSemantics(mapped, allowedToolsFilter, {
				hasNativeToolFilter,
			});
		}
		...
		// Codex than Claude-style Miko allowedTools entries. A bare
