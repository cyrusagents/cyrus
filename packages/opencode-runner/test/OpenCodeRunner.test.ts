...
function makeTempDir(): string {
	return mkdtempSync(join(tmpdir(), "miko-opencode-runner-"));
}
...
describe("OpenCodeRunner", () => {
	it("spawns opencode run with JSON output flags and maps replay events to Miko messages", async () => {
	...
	it("rejects Miko-style OpenCode model selectors before spawning OpenCode", async () => {
		const dir = makeTempDir();
		const captureFile = join(dir, "capture.json");
		const opencodePath = writeFakeOpenCode(
			dir,
		...
		expect(errors[0]?.message).toBe(
			'Invalid OpenCode model selector "opencode/kimi-k2.7-code". Use a provider-qualified OpenCode model such as "openai/gpt-5.5" in runner config or select it with the Miko label "opencode/openai/gpt-5.5".',
		);
	...
	it("coerces realistic OpenCode JSON events into Miko messages and final result", async () => {
		const dir = makeTempDir();
		const opencodePath = writeFakeOpenCode(
			dir,
...
`,
	...
	it("does not copy Miko skills or synthesize bootstrap skills into OpenCode config", async () => {
		const dir = makeTempDir();
		const pluginPath = join(dir, "miko-skills-plugin");
