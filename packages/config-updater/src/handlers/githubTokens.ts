 ...
 * `<mikoHome>/scripts/gh-miko.cjs`. The droplet's `~/.local/bin/gh`
 ...
 */
export function ensureGhTokenResolver(mikoHome: string): string {
	const scriptDir = join(mikoHome, "scripts");
	const scriptDest = join(scriptDir, "gh-miko.cjs");
	mkdirSync(scriptDir, { recursive: true });
	copyFileSync(bundledScriptPath("gh-miko.cjs"), scriptDest);
	chmodSync(scriptDest, 0o755);
	return scriptDest;
}

/**
 * Install the Miko git credential helper and wire it into the global git
 ...
 *   `<mikoHome>/scripts/git-credential-miko.cjs` (executable).
 ...
 *   with an empty entry followed by the Miko helper. Helper values must be
 ...
 */
export function ensureGitHubCredentialHelper(mikoHome: string): string {
	const scriptDir = join(mikoHome, "scripts");
	const scriptDest = join(scriptDir, "git-credential-miko.cjs");

	mkdirSync(scriptDir, { recursive: true });
	copyFileSync(bundledScriptPath("git-credential-miko.cjs"), scriptDest);
	chmodSync(scriptDest, 0o755);
...
/**
 * Self-heal the droplet's `~/.local/bin/gh` wrapper to exec the Miko gh
 * token resolver.
 *
 * Droplet images bake a gh wrapper that strips injected GH_TOKEN /
 * GITHUB_TOKEN env vars. The current design routes gh through
 * `<mikoHome>/scripts/gh-miko.cjs`, which resolves the installation
 ...
 * since the wrapper lives in the miko user's home, rewrite it here on
 ...
 */
export function ensureGhWrapperSupportsMikoToken(
	...
	// strip-everything wrapper and the interim MIKO_GH_TOKEN one), and
	// only when they predate the resolver.
	if (current.includes("gh-miko.cjs") || !current.includes("/usr/bin/gh")) {
		return false;
	}

	const updated = `#!/usr/bin/env bash
# Miko-managed gh wrapper. The resolver picks the GitHub App installation
...
RESOLVER="$HOME/.miko/scripts/gh-miko.cjs"
if [ -f "$RESOLVER" ]; then
  exec node "$RESOLVER" "$@"
fi
 ...
 * 20 minutes; with refresh moved to miko-hosted, this keeps bare `gh`
...
/**
 * Handle a GitHub installation tokens push from miko-hosted.
 *
 * Persists the per-installation tokens to `<mikoHome>/github-tokens.json`
...
export async function handleGitHubTokens(
	...
	try {
		new GitHubTokenStore(mikoHome).save(payload.tokens);
	} catch (error) {
		return {
			success: false,
	...
	try {
		...
		// Non-fatal: gh falls back to MIKO_GH_TOKEN / hosts.yml auth.
		console.warn(
	...
	try {
		// mikoHome is <home>/.miko on droplets, so the wrapper lives at
		// <parent of mikoHome>/.local/bin/gh. Using the parent (rather than
		// os.homedir()) keeps this no-op for custom miko-home layouts and
		// hermetic in tests.
		ensureGhWrapperSupportsMikoToken(dirname(mikoHome));
	} catch (error) {
