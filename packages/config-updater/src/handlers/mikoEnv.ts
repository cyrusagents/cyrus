...
import type { ApiResponse, MikoEnvPayload } from "../types.js";

/**
 * Handle Miko environment variables update
 * Primarily used to update/provide the Claude API token
 */
export async function handleMikoEnv(
	...
	try {
		...
		const envVars = Object.entries(envVarsSource).filter(
			([key, value]) =>
				value !== undefined &&
				typeof value === "string" &&
				!["variables", "restartMiko", "backupEnv"].includes(key),
		) as [string, string][];
		...
		const envPath = join(mikoHome, ".env");
		...
		if (payload.backupEnv && existsSync(envPath)) {
			try {
				...
				const backupPath = join(mikoHome, `.env.backup-${timestamp}`);
		...
		try {
			...
			return {
				...
				data: {
					...
					restartMiko: payload.restartMiko || false,
				},
			};
		} catch (error) {
			return {
