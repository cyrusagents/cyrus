...
function runHelper(op: string, stdin: string, mikoHome: string): HelperResult {
...
describe("git-credential-miko helper script", () => {
	let mikoHome: string;

	beforeEach(() => {
		mikoHome = mkdtempSync(join(tmpdir(), "miko-cred-script-"));
	});
