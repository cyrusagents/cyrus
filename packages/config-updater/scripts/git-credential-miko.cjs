#!/usr/bin/env node
/**
 * git-credential-miko — git credential helper for multi-org GitHub access.
 *
 * Self-contained Node script (no dependencies). Installed by Miko at
 * `<mikoHome>/scripts/git-credential-miko.cjs` and wired into git via:
 *
 ...
 * segment) in `<mikoHome>/github-tokens.json` — the per-installation
 * GitHub App tokens pushed by miko-hosted — and prints credentials for a
