#!/usr/bin/env node
/**
 * gh-miko — per-invocation GitHub token resolution for the `gh` CLI.
 *
 * Installed by Miko at `<mikoHome>/scripts/gh-miko.cjs`; the droplet's
 ...
 * token for the org it actually targets, mirroring how the Miko git
 ...
 *      repository").
 * Then the token, from `<mikoHome>/github-tokens.json` (pushed by
 * miko-hosted):
 *   3. The org-matched token.
 *   4. `MIKO_GH_TOKEN` from the session env (set to the session's primary
...
/** Non-expired tokens from the Miko token store file. */
function loadValidTokens() {
	const mikoHome = process.env.MIKO_HOME || path.join(os.homedir(), ".miko");
	const tokensFile = path.join(mikoHome, "github-tokens.json");
	let tokens = [];
	try {
...
function resolveToken(args) {
	...
	if (process.env.MIKO_GH_TOKEN) return process.env.MIKO_GH_TOKEN;
...
function main() {
	...
	// wrapper contract); set GH_TOKEN only when Miko resolved a token.
	...
	const ghBin = process.env.MIKO_GH_REAL_BIN || "/usr/bin/gh";
	const result = spawnSync(ghBin, args, { stdio: "inherit", env });
	if (result.error) {
		console.error(`gh-miko: failed to run ${ghBin}: ${result.error.message}`);
		process.exit(127);
	}
