...
describe("github-webhook-utils", () => {
	...
	describe("stripMention", () => {
		it("strips @mikoagent from comment body", () => {
			expect(stripMention("@mikoagent Please fix the failing tests")).toBe(
				"Please fix the failing tests",
			);
		});

		it("strips mention from middle of text", () => {
			expect(stripMention("Hey @mikoagent please fix this")).toBe(
				"Hey please fix this",
			);
		});

		it("handles case-insensitive mentions", () => {
			expect(stripMention("@MikoAgent Please fix")).toBe("Please fix");
		});
		...
		it("handles text that is only the mention", () => {
			expect(stripMention("@mikoagent")).toBe("");
		});
