...
describe("GitHubEventTransport", () => {
	...
	describe("runtime mode switching (proxy → signature)", () => {
		...
		afterEach(() => {
			...
			delete process.env.MIKO_HOST_EXTERNAL;
		});

		it("switches to signature verification when GITHUB_WEBHOOK_SECRET and MIKO_HOST_EXTERNAL are set at request time", async () => {
			const eventListener = vi.fn();
			transport.on("event", eventListener);

			// Add env vars after startup
			process.env.GITHUB_WEBHOOK_SECRET = runtimeWebhookSecret;
			process.env.MIKO_HOST_EXTERNAL = "true";
		...
		it("stays in proxy mode when only GITHUB_WEBHOOK_SECRET is set (no MIKO_HOST_EXTERNAL)", async () => {
			const eventListener = vi.fn();
			transport.on("event", eventListener);
