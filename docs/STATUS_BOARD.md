...
Start Miko normally, then open [http://127.0.0.1:3456/board](http://127.0.0.1:3456/board).
...
For this fork, follow [Fork Installation](./FORK_INSTALLATION.md) and start it with the verified source launcher. Downloading the setup skills alone does not install the runtime; run `/miko-setup` to build it.

The English interface has a task list and a live log pane. The default **Activity** view shows
compact, color-coded assistant, tool, result and Miko rows. Tool calls and results appear together
...
- The board includes issue sessions and chat sessions managed by this Miko instance.
...
- Miko logs come from its structured logger, starting when the board is registered. Historical stdout/stderr files are not scanned.
...
- The archive is stored in `<mikoHome>/state/board-history.json` with atomic writes. It keeps up to 200 removed tasks and the latest 100 public agent log entries per task, within 16 MiB overall; the oldest tasks are evicted first. Storage failures show a warning and do not stop Miko.
...
The UI source is in `packages/edge-worker/board/`. Run `pnpm --filter miko-edge-worker build:board`
